...
function Campusevent_login()
 {
    ...
    function onLoginFormSubmit(userCredential)
    {
      ...
      .then((userarray)=>{
        ...
        else
        {
          ...
          if(result===true)
          {
          navigate(`/campusevent_admin/${userCredential.username}`,{state:userarray[0]})
          }
          else{
  ...
  return (
    <div className='d-block mx-auto w-50 bg-secondary p-4 mt-5' >
        <h1 className='fs-1 text-warning text-center fw-bold'>Campus Event Login</h1>
...
export default Campusevent_login
